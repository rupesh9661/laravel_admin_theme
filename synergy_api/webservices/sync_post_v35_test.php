...
//        $CometitorNewLaunchProductReport = $data->response->CometitorNewLaunchProductReport;
        ...
        $SSPrimarySaleSummary = $data->response->SSPrimarySaleSummary;
        $SSPrimarysaledetail = $data->response->SSPrimarySaleDetail;

         $SSCounterSaleSummary = $data->response->SSCounterSaleSummary;
        $SSCounterSaleDetail = $data->response->SSCounterSaleDetail;


          if(!empty($SSCounterSaleDetail))
        {
            foreach ($SSCounterSaleDetail as $key => $c_value_s) 
            {
                $order_id = $c_value_s->order_id;
                $ss_id = $c_value_s->dealer_id;
                ...
                $track_date = date('Y-m-d', strtotime($date_time));
                $track_time = date('H:i:s', strtotime($date_time));



                    $counter_sale_data = "INSERT INTO `ss_counter_sale_summary`(`order_id`, `csa_id`, `sale_date`, `created_date`, `date_time`, `battery_status`, `gps_status`, `lat`, `lng`, `mcc_mnc_lac_cellid`, `company_id`, `address`, `created_by_person`,`server_date`) VALUES ('$order_id','$ss_id','$sale_date','$created_date','$date_time','$battery_status','$gps_status','$lat','$lng','$mcc_mnc_lac_cellid','$company_id','$address','$user_id',NOW())";
                    $counter_sale_data_run = mysqli_query($dbc, $counter_sale_data);

                    $market_track="INSERT INTO `user_work_tracking`(`user_id`, `track_date`, `track_time`, `mnc_mcc_lat_cellid`, `lat_lng`, `track_address`, `status`, `server_date_time`,`battery_status`,`gps_status`,`company_id`) VALUES ('$user_id','$track_date','$track_time','$mcc_mnc_lac_cellid','$latlng','$address','SS Counter Sale',NOW(),'$battery_status','$gps_status','$company_id')";
        ...
        if(!empty($SSCounterSaleSummary))
        {
            foreach ($SSCounterSaleSummary as $counter_key => $counter_value) 
            {
                ...
                $weight_rate = !empty($counter_value->weight_rate)?$counter_value->weight_rate:'0';
                $weight_qty = !empty($counter_value->weight_qty)?$counter_value->weight_qty:'0';



                $chk_usod = "SELECT id FROM ss_counter_sale_details WHERE order_id='$order_id' AND product_id='$product_id' AND `company_id` = '$company_id'";
                ...
                $counter_details_sale_data = "INSERT INTO `ss_counter_sale_details`(`order_id`, `product_id`, `rate`, `quantity`, `barcode`, `cases`, `pcs`, `value`, `case_rate`, `pcs_rate`, `company_id`,`created_by`,`server_date_time`,`weight_rate`,`weight_qty`) VALUES ('$order_id','$product_id','$rate','$quantity','$barcode','$case','$pcs','$value','$case_rate','$pcs_rate','$company_id','$user_id',NOW(),'$weight_rate','$weight_qty')";
         ...
         if (!empty($SSPrimarySaleSummary)) {
            //print_r($PrimarySaleSummary);
            $psale = count($SSPrimarySaleSummary);
            $ps = 0;
            while ($ps < $psale) {
                $order_id = $SSPrimarySaleSummary[$ps]->order_id;
                $product_id = $SSPrimarySaleSummary[$ps]->product_id;
                $rate = $SSPrimarySaleSummary[$ps]->pcs_rate;  //app side sending pcs_rate as pcs rate(dealer rate)
                $quantity = $SSPrimarySaleSummary[$ps]->quantity;
                $scheme_qty = !empty($SSPrimarySaleSummary[$ps]->scheme_qty)?$SSPrimarySaleSummary[$ps]->scheme_qty:'0';
                $case = $SSPrimarySaleSummary[$ps]->case;
                $pcs = $SSPrimarySaleSummary[$ps]->pcs;
                $case_rate = $SSPrimarySaleSummary[$ps]->case_rate;
                $pcs_rate = $SSPrimarySaleSummary[$ps]->pcs_rate;

                $weight_rate = !empty($SSPrimarySaleSummary[$ps]->weight_rate)?$SSPrimarySaleSummary[$ps]->weight_rate:'0';
                $weight_qty = !empty($SSPrimarySaleSummary[$ps]->weight_qty)?$SSPrimarySaleSummary[$ps]->weight_qty:'0';


                 $chk_usod = "SELECT id FROM ss_user_primary_sales_order_details WHERE order_id='$order_id$user_id' AND product_id='$product_id' AND `company_id` = '$company_id' ";
                $run_uhod = mysqli_query($dbc, $chk_usod);
                $num_data = mysqli_num_rows($run_uhod);
                if ($num_data < 1) {

                 $q = "INSERT INTO `ss_user_primary_sales_order_details`(`id`, `order_id`, `product_id`, `rate`, `quantity`, `scheme_qty`,`cases`,`pcs`,`pr_rate`,`company_id`,`weight_rate`,`weight_qty`) "
                    . " VALUES('$order_id$ps','$order_id$user_id','$product_id','$rate','$quantity','$scheme_qty','$case','$pcs','$case_rate','$company_id','$weight_rate','$weight_qty')";
                    // die;
        ...
        if (!empty($SSPrimarysaledetail)) {
            $psalesum = count($SSPrimarysaledetail);
            $pd = 0;
            while ($pd < $psalesum) {
                $order_id = $SSPrimarysaledetail[$pd]->order_id;
                $csa_id = $SSPrimarysaledetail[$pd]->s_id;
                $created_date = $SSPrimarysaledetail[$pd]->created_date;
                $sale_date = $SSPrimarysaledetail[$pd]->sale_date;
                $date_time = $SSPrimarysaledetail[$pd]->date_time;
                $lat = $SSPrimarysaledetail[$pd]->lat;
                $lng = $SSPrimarysaledetail[$pd]->lng;
                $lat_lng = $lat.','.$lng;
                $address = $SSPrimarysaledetail[$pd]->address;
                $gps_status = $SSPrimarysaledetail[$pd]->gps_status;
                $mcc_mnc_lac_cellid = !empty($SSPrimarysaledetail[$pd]->mcc_mnc_lac_cellid)?$SSPrimarysaledetail[$pd]->mcc_mnc_lac_cellid:'NA';
                $battery_status = $SSPrimarysaledetail[$pd]->battery_status;
                ...
                $track_date = date('Y-m-d', strtotime($date_time));
                    ...
                    $q = "INSERT INTO `ss_user_primary_sales_order`(`id`,`order_id`, `dealer_id`, `created_date`, `created_person_id`, `sale_date`, `receive_date`, `date_time`,`lat`,`lng`,`address`,`company_id`)"
                        . " VALUES('$order_id$user_id','$order_id$user_id','$csa_id','$created_date','$user_id','$sale_date',NOW(),'$date_time','$lat','$lng','$address','$company_id')";
    //                echo $q;die;
                    $primary_order_track="INSERT INTO `user_work_tracking`(`user_id`, `track_date`, `track_time`, `mnc_mcc_lat_cellid`, `lat_lng`, `track_address`, `status`, `server_date_time`,`battery_status`,`gps_status`,`company_id`) VALUES ('$user_id','$track_date','$track_time','$mcc_mnc_lac_cellid','$lat_lng','$address','SS Primary Booking',NOW(),'$battery_status','$gps_status','$company_id')";
...
//                echo $cmp_query;die;
        ...
        if (!empty($dailyReporting)) {
            foreach ($dailyReporting as $newda) {
                $dr_query = "INSERT INTO `daily_reporting`(`user_id`, `work_date`, `server_date_time`, `work_status`, `working_with`, `user_location`, `mnc_mcc_lat_cellid`, `lat_lng`, `remarks`, `attn_address`, `order_id`, `dealer_id`, `location_id`,`company_id`) 
                VALUES ('$user_id','$newda->date_time',NOW(),'$newda->status','$newda->working_with','$newda->location','$newda->check_mcc_mnc_lac_cellId_final','$newda->latlng','$newda->remark','$newda->location','$newda->orderid','$newda->dealerid','$newda->LocId','$company_id')";
...
//echo $q;
        ...
        if (!empty($BalanceStock)) {
            ...
            while ($bre < $count_BalanceStock) {
                ...
                $qbr = "INSERT INTO `dealer_balance_stock`(`order_id`, `dealer_id`, `user_id`, `product_id`, `stock_qty`, `mfg_date`, `exp_date`, `cases`, `submit_date_time`, `server_date_time`,`mrp`,`pcs_mrp`,sstatus,`lat`,`lng`,`address`,`company_id`) VALUES ('$balance_order_id$user_id','$dealer_id','$user_id','$balance_product_code','$balance_pieces','$mfg_date','$exp_date','$balance_cases','$mobile_datetime',NOW(),'$bsmrp','$balance_pcs_mrp','0','$lat','$lng','$address','$company_id')";
...
//echo $qbr;
        ...
        if (!empty($retailerstockdetails)) {
            ...
            while ($j < $count_retailerstockdetails) {
                ...
                $q = "INSERT INTO `retailer_stock_details`(`order_id`, `product_id`, `quantity`,`stock_month`,`company_id`) VALUES  ('$order_id','$product_id','$qty','$stock_month','$company_id')";
                $result = mysqli_query($dbc, $q);
                if ($result) {
                    $unique_id_array[] = $order_id;
        ...
        if (isset($attendance) && !empty($attendance)) {
            ...
            while ($k < $attcount) {
                ...
                $num = mysqli_num_rows($sql);
                if ($num < 1) {
                    $q = "INSERT INTO `user_daily_attendance`(`user_id`, `order_id`, `work_date`,`working_with`, `work_status`,`mnc_mcc_lat_cellid`, `lat_lng`, `track_addrs`, `remarks`,`server_date`,`battery_status`,`gps_status`,`company_id`)VALUES
                ('$user_id','$order_id$user_id','$date_time','$working_with','$status','$mnc_mcc_lat_cellid','$latlng','$user_location','$remark',CURRENT_TIMESTAMP,'$battery_status','$gps_status','$company_id')";
                    $run = mysqli_query($dbc, $q);
                    if ($run) {
                        $unique_id_array[] = $order_id;
        ...
        if (!empty($Checkoutlocation)) {
            ...
            while ($c < $checkcount) {
                ...
                $num = mysqli_num_rows($sql);
                if ($num < 1) {
                     $q = "INSERT INTO check_out(`user_id`,`lat_lng`,`mnc_mcc_lat_cellid`,`work_date`,`server_date_time`,`attn_address`,`order_id`,`remarks`,`total_call`,`total_pc`,`total_sale_value`,`battery_status`,`gps_status`,`company_id`)
                        VALUES('$user_id','$latlng','$mcc_mnc_lac_cellId_final','$date_time','$current_date_time','$user_location','$order_id$user_id','$remarks','$total_call','$total_pc','$total_sale_value','$battery_status','$gps_status','$company_id')";
                    #update user_daily_attendance with checkout
...
///////////////////////////////////////////////////////////////////////////////
        if (!empty($callwisereportingstatus)) {
            ...
            while ($p < $callwisereport) {
                ...
                $num_data = mysqli_num_rows($run_uhod);
                if ($num_data < 1) {
                    $qusod = "INSERT INTO `user_sales_order_details`(`order_id`, `product_id`, `rate`, `quantity`, `scheme_qty`,`company_id`)"
                        . " VALUES('$orderid$user_id','$product_id','$rate','$prod_qty','$scheme_qty','$company_id')";
        ...
        if (!empty($callwisereporting)) {
            ...
            while ($l < $phonestatus) {
                ...
                if ($num_uso < 1) {
                    $quso = "INSERT INTO `user_sales_order`(`order_id`, `user_id`, `dealer_id`, `location_id`, `retailer_id`,`call_status`, `total_sale_value`,`discount`,`amount`, `total_sale_qty`,`lat_lng`, `mccmnclatcellid`,`geo_status`,`track_address`, `date`, `time`, `image_name`, `override_status`,`remarks`,`battery_status`,`gps_status`,`reason`,`company_id`)"
                        . " VALUES('$order_id$user_id','$user_id','$dealer_id','$beat_id','$retailer_id','$call_status','$total_sale_value','$discount','$discount_before','$total_sale_qty','$lat_lng','$mcc_mnc','$geo_status','$user_location','$date','$time','.jpg','$override_status','$remarks','$battery_status','$gps_status','$non_productive_id','$company_id')";
                    //      echo $q;
        ...
        if (!empty($PrimarySaleSummary)) {
            ...
            while ($ps < $psale) {
                ...
                $pcs_rate = $PrimarySaleSummary[$ps]->pcs_rate;
                 ...
                 $q = "INSERT INTO `user_primary_sales_order_details`(`id`, `order_id`, `product_id`, `rate`, `quantity`, `scheme_qty`,`cases`,`pcs`,`pr_rate`,`company_id`) "
        ...
        if (!empty($Primarysaledetail)) {
            ...
            while ($pd < $psalesum) {
                ...
                $q = "INSERT INTO `user_primary_sales_order`(`id`,`order_id`, `dealer_id`, `created_date`, `created_person_id`, `sale_date`, `receive_date`, `date_time`,`lat`,`lng`,`address`,`company_id`)"
                    . " VALUES('$order_id$user_id','$order_id$user_id','$dealer_id','$created_date','$user_id','$sale_date',NOW(),'$date_time','$lat','$lng','$address','$company_id')";
//                echo $q;die;
                $result = mysqli_query($dbc, $q);
                if ($result) {
                    $unique_id_array[] = $order_id;
        ...
        if (!empty($createcustomer)) {
            ...
            while ($cc < $customer) {
               ...
               $sequence = ($sequence_id->retailer_code)+1;
...
//                }
        ...
        if (!empty($tracking)) {
            ...
            while ($tr < $track) {
                ...
                $num = $sql['num'];
                if ($num < 1) {
                    $q = "INSERT INTO `user_daily_tracking`(`user_id`, `track_date`, `track_time`,`mnc_mcc_lat_cellid`, `lat_lng`, `track_address`,`battery_status`,`gps_status`,`company_id`)"
                        . " VALUES('$user_id','$track_date','$track_time','$mnc_mcc_lat_cellid','$lat_lng','$user_location','$battery_status','$gps_status','$company_id')";
                    $result = mysqli_query($dbc, $q);
...
//////////////////////////////////RETAILER Reshuffle/////////////////////////
        if (isset($RetailerReshuffle) && !empty($RetailerReshuffle)) {
            ...
            while ($retres < $RetailerReshuffle_count) {
                ...
                $qryretres = "UPDATE `user_dealer_retailer` SET `seq_id`='$retailer_res_seq_new',`udr_date_time`=NOW() WHERE user_id='$user_id' AND dealer_id='$retailer_dealer_id' AND retailer_id='$retailer_res_id' AND `company_id` = '$company_id' ";

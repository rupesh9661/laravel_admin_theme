...
//=================================================================================
     ...
     $q = "SELECT * From employee_info WHERE user_primary_id='$user_id' AND `company_id` = '$company_id'";

    $user_res = mysqli_query($dbc, $q);
    $q_person = mysqli_fetch_assoc($user_res);
    // print_r($q_person);
    $person_id = $q_person['user_primary_id'];
    $status = $q_person['status'];
        ...
        if(!empty($MarketingActivity))
        {
            ...
            foreach ($MarketingActivity as $mar_key => $mar_value) 
            {
                ...
                // $marketActArray[] = "('$user_id','$cur_date','$cur_time','$mcc_mnc_lac_cellid','$latlng','$geoaddress','Marketing Activity',NOW(),'$battery_status','$gps_status','$company_id')";
                  ...
                  $marketing__activitity_track="INSERT INTO `user_work_tracking`(`user_id`, `track_date`, `track_time`, `mnc_mcc_lat_cellid`, `lat_lng`, `track_address`, `status`, `server_date_time`,`battery_status`,`gps_status`,`company_id`) VALUES ('$user_id','$cur_date','$cur_time','$mcc_mnc_lac_cellid','$latlng','$geoaddress','Marketing Activity',NOW(),'$battery_status','$gps_status','$company_id') ";
...
///////////////////////////////////////////////////////////////////////////////
        ...
        if (!empty($callwisereportingstatus)) {
            ...
            while ($p < $callwisereport) {
                ...
                $weight_rate = !empty($callwisereportingstatus[$p]->weight_rate)?$callwisereportingstatus[$p]->weight_rate:'0';
                $weight_qty = !empty($callwisereportingstatus[$p]->weight_qty)?$callwisereportingstatus[$p]->weight_qty:'0';
                ...
                if ($run_assoc_uho >= 1) {
                    ...
                    if ($num_data < 1 ) {
                        ...
                        $qusod = "INSERT INTO `user_sales_order_details`(`order_id`, `product_id`, `rate`,`case_rate`, `quantity`, `scheme_qty`,`company_id`,`piece_quantity`,`case_quantity`,`secondary_quantity`,`secondary_rate`,`weight`,`status`,`weight_rate`,`weight_qty`)"
                            . " VALUES('$orderid$user_id','$product_id','$rate','$case_rate','$prod_qty','$scheme_qty','$company_id','$piece_quantity','$case_quantity','$secondary_quantity','$secondary_rate','$pro_weight','0','$weight_rate','$weight_qty')";
        ...
        if (!empty($PrimarySaleSummary)) {
            ...
            while ($ps < $psale) {
                ...
                $weight_qty = !empty($PrimarySaleSummary[$ps]->weight_qty)?$PrimarySaleSummary[$ps]->weight_qty:'0';
                $weight_rate = !empty($PrimarySaleSummary[$ps]->weight_rate)?$PrimarySaleSummary[$ps]->weight_rate:'0';



                 $q = "INSERT INTO `user_primary_sales_order_details`(`id`, `order_id`, `product_id`, `rate`, `quantity`, `scheme_qty`,`cases`,`pcs`,`pr_rate`,`company_id`,`secondary_qty`,`weight`,`weight_rate`,`weight_qty`) "
                    . " VALUES('$order_id$ps','$order_id$user_id','$product_id','$rate','$quantity','$scheme_qty','$case','$pcs','$case_rate','$company_id','$secondary_qty','$pro_weight','$weight_rate','$weight_qty')";
                    // die;
                $result = mysqli_query($dbc, $q);

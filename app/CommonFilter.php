...
class CommonFilter extends Model
{
    ...
    {   
        ...
        $location3data =DB::table('employee_info')
        ->join('users','employee_info.user_primary_id','=','users.id')
        ->select(DB::raw('CONCAT(f_name," ",l_name) as name'), 'users.id as uid')
        ->where('employee_info.status','=','1')
        ->where('users.company_id',$company_id)
        ->where('employee_info.company_id',$company_id);
        
        $location3 = $location3data->pluck('name', 'uid');
         
         return $location3;
    }
    public static function user_filter_deactivated($var)
    {   
        // $company_id = Auth::user()->company_id;
        // // $var = str_replace("'",'', $var);
        // // dd($var);
